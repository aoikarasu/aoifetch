...
export function getTerminalName() {
  ...
  // Secondary label for parentheses
  let meta = null;
  if (env.TERMUX_VERSION) meta = 'Termux';
  else if (env.KONSOLE_VERSION) meta = 'Konsole';
    ...
    meta = map[env.TERM_PROGRAM] || env.TERM_PROGRAM;
  }

  // For Termux, Linux, and macOS: prefer $TERM, with meta in parentheses
  if (platform === 'termux' || platform === 'android' || platform === 'linux' || platform === 'darwin') {
    const term = env.TERM;
    if (term) return `${term}${meta ? ` (${meta})` : ''}`;
    // fallback if TERM is absent
    if (meta) return meta;
    if (env.TERMINAL_EMULATOR) return `${env.TERMINAL_EMULATOR}`;
    return 'unknown';
  }

  // Windows and other platforms: retain previous behavior
