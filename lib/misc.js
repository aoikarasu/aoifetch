...
export function getUptime(os) {
  ...
  let parts = [];
  if (days > 0) parts.push(`${y(days)} day${days !== 1 ? 's' : ''}`);
  if (hours > 0) parts.push(`${y(hours)} hour${hours !== 1 ? 's' : ''}`);
  parts.push(`${y(minutes)} min${minutes !== 1 ? 's' : ''}`);
  return parts.join(", ");
}
...
export function parseListFormat(output, delimeter = '=') {
  ...
  for (const line of lines) {
    ...
    const idx = line.indexOf(delimeter);
    if (idx > -1) {
