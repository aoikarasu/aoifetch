  ...
  if (platform == 'termux') {
    try {
      const output = execSync(
        'getprop ro.build.version.release && getprop ro.build.version.codename && getprop ro.build.version.sdk'
      ).toString().trim().split('\n');

      const [version, codename, sdk] = output;
      pos.pretty = () => `Android ${version} ${codename}, API Level ${sdk}`;
    }
    catch { }
  }
